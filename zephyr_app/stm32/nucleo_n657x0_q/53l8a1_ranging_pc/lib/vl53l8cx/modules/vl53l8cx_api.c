		...
		uint8_t					expected_value)
{
	...
	do {
		...
		{
			printk("[vl53l8cx] poll_for_answer TIMEOUT reg=0x%04x mask=0x%02x"
			       " expected=0x%02x got=0x%02x\n",
			       address, mask, expected_value, p_dev->temp_buffer[pos]);
			status |= (uint8_t)VL53L8CX_STATUS_TIMEOUT_ERROR;
			break;
		}else if((size >= (uint8_t)4)
                         && (p_dev->temp_buffer[2] >= (uint8_t)0x7f))
		{
			printk("[vl53l8cx] MCU_ERROR reg=0x%04x buf[2]=0x%02x\n",
			       address, p_dev->temp_buffer[2]);
              ...
              VL53L8CX_Configuration      *p_dev)
{
   ...
   do {
	...
	if (timeout >= (uint16_t)500) {
		printk("[vl53l8cx] poll_for_mcu_boot TIMEOUT go2_status0=0x%02x\n",
		       go2_status0);
	}
		...
		uint8_t						resolution)
{
	...
	printk("[dbg] offset write[0..7]: %02x %02x %02x %02x %02x %02x %02x %02x\n",
		p_dev->temp_buffer[0], p_dev->temp_buffer[1],
		p_dev->temp_buffer[2], p_dev->temp_buffer[3],
		p_dev->temp_buffer[4], p_dev->temp_buffer[5],
		p_dev->temp_buffer[6], p_dev->temp_buffer[7]);
