...
#define TOTAL_ZONES         (ZONES_PER_LINE * ZONES_PER_LINE)
...
static void fill_frame_from_results(const VL53L8CX_ResultsData *results,
                                    struct ranging_frame *frame)
{
    frame->timestamp_ms = k_uptime_get();

    for (int zone = 0; zone < TOTAL_ZONES; zone++) {
        uint8_t detected = results->nb_target_detected[zone] > 0U;
        frame->target_detected[zone] = detected;
        frame->distance_mm[zone] = detected
            ? results->distance_mm[zone * VL53L8CX_NB_TARGET_PER_ZONE]
            : -1;
    }
}
...
int main(void)
{
    ...
    /* --- Initialize network --- */
    printk("\n");
    int net_ret = net_client_init();
    if (net_ret != 0) {
        printk("WARN: Network init failed, continuing without streaming\n");
    } else {
        net_ret = net_client_start();
        if (net_ret != 0) {
            printk("WARN: network client start failed (%d), stream disabled\n", net_ret);
        } else {
            printk("Network client task started\n");
        }
    }

    if (net_ret != 0) {
        printk("NET: streaming disabled until network is available\n");
    } else {
        printk("NET: streaming enabled\n");
    }
    printk("\n");
    ...
    while (1) 
    {
        ...
        if (data_ready) 
        {
            ...
            fill_frame_from_results(&results, &frame);
            net_client_submit(&frame);
        }
        k_msleep(100);
    }
