/*
 * VL53L8CX platform header — Zephyr SPI port
 *
 * Replaces the original function-pointer based platform struct with
 * a Zephyr spi_dt_spec-based struct. The sensor API (vl53l8cx_api.c)
...
typedef struct {
    ...
    /* Unused on SPI — present only because vl53l8cx_api.c's
     * vl53l8cx_set_i2c_address() unconditionally writes
     * p_dev->platform.address (vendor code, compiled regardless of bus). */
    uint16_t address;
} VL53L8CX_Platform;
