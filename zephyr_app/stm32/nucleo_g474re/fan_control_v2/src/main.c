...
int main(void)
{
	...
	/*
	 * Startup got this far without a hard failure above: confirm this image so
	 * MCUboot stops treating it as a trial upgrade. Without this, the next
	 * reset after an update reverts to the previous image (swap-using-offset
	 * mode). No-op if the image is already confirmed (e.g. not from an update).
	 */
	ret = boot_write_img_confirmed();
	if (ret != 0) {
		printf("Failed to confirm image: %d\n", ret);
	}
